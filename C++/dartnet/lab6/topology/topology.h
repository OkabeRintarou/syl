...
void* checkedRealloc(void* ptr,size_t new_size);
...
void initDArray(darray_t* d,int num,int itembytes);
void destroyDArray(darray_t* d);

typedef struct topologyneighbor{
    int neighborNum;
    darray_t neighbornames;
    darray_t costs;
}topologyneighbor_t;


typedef struct topologyentry{
    char* hostname;
    darray_t neighbors;
}topologyentry_t;
